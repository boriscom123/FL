$(document).ready(function () {
   ...
   let bg1 = document.querySelector('.shape1');
   let bg2 = document.querySelector('.shape2');
   let bg3 = document.querySelector('.shape3');
   let bg4 = document.querySelector('.shape4');
   let bg5 = document.querySelector('.shape5');

   window.addEventListener('mousemove', function(e) {
      let x = e.clientX / window.innerWidth;
      let y = e.clientY / window.innerHeight;  
      if(bg1) bg1.style.transform = 'translate(-' + x * 50 + 'px, -' + y * 20 + 'px)';
      if(bg2) bg2.style.transform = 'translate(-' + x * 40 + 'px, -' + y * 30 + 'px)';
      if(bg3) bg3.style.transform = 'translate(-' + x * 30 + 'px, -' + y * 50 + 'px)';
      if(bg4) bg4.style.transform = 'translate(-' + x * 20 + 'px, -' + y * 40 + 'px)';
      if(bg5) bg5.style.transform = 'translate(-' + x * 10 + 'px, -' + y * 10 + 'px)';
   });
   ...
   $(document).on('click','.buy-payment-goods', function (e) { 
      ...
      $(".buy-payment-goods").prop('disabled', true);
        ...
        $.ajax({type: "POST",url: url_path + "systems/ajax/ads.php",data: "id="+id+"&action=buy_payment_goods",dataType: "json",cache: false,success: function (data) { 
            if( data["status"] == true ){
               $(".redirect-form-pay").html(data["redirect"]);
   ...
   $(document).on('click','.mobile-sidebar-menu', function (e) { 

      if (!$(e.target).closest(".mobile-sidebar-menu-content").length) {
         $(this).hide();
         $("body").css("overflow", "auto");
      }
      e.stopPropagation();
    ...
    $('.ajax-live-search').keyup(function() {
      ...
      adSearchTimeout = setTimeout(function() {
        adSearchTimeout = null;  
          ...
          $.ajax({type: "POST",url: url_path + "systems/ajax/ads.php",data: "query=" + $('.ajax-live-search').val() + "&action=ads_search",dataType: "html",cache: false,                        
              success: function (data){
                  if(data){
