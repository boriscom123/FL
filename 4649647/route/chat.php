...
if( !$_SESSION['profile']['id'] ){ header( "Location: " . _link("auth") ); }
...
$data["share"] = $Main->share( array( "title" => "Я размещаю объявления на ".$settings["site_name"].". Посмотрите, возможно, вас заинтересуют мои предложения.", "image" => $Profile->userAvatar($user["clients_avatar"]), "url" => _link( "user/".$id_user ) ) );
